...
//used to fill up the matrix with recursive calls
//auxillary function for knapsack
int getKnapsackVal(vector<vector<int>> & knapsack, int i, int c, Problem problem){
  int wi = problem.weights[i-1]; //weight of last item
  int pi = problem.values[i-1]; //value of last item
  if(knapsack[i][c]>=0) return knapsack[i][c];
    ...
    int k1 = getKnapsackVal(knapsack, i-1, c, problem);//knapsack for one of the max considerations
    int k2 = getKnapsackVal(knapsack, i-1, c-wi, problem); //knapsack for the other max consideration
...
//Recursive with memo solution to the knapsack problem
